...
".picker-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.45); display: none; align-items: center; justify-content: center; z-index: 999; }"
".picker-dialog { background: #ffffff; border-radius: 12px; padding: 14px 16px; width: 260px; box-shadow: 0 8px 24px rgba(0,0,0,0.35); text-align: center; animation: popIn 0.25s ease-out; }"
"@keyframes popIn { from { opacity: 0; transform: scale(0.9); } to { opacity: 1; transform: scale(1); } }"
".picker-title { font-weight: bold; margin-bottom: 10px; color: #333; }"
".picker-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-bottom: 10px; }"
".picker-btn { padding: 8px; font-size: 1.4em; border-radius: 8px; border: 1px solid #764ba2; background: #f7f5ff; cursor: pointer; transition: all 0.15s; }"
".picker-btn:active { transform: scale(0.95); background: #e8e4ff; }"
".picker-cancel { padding: 6px 10px; font-size: 0.85em; border-radius: 999px; border: 1px solid #bbb; background: #f5f5f5; cursor: pointer; }"
"</style>"
"</head>"
"<body>"
"<div class='container'>"
...
"<div class='roll-info' id='rollInfo'>Tap a die to choose its value (1-6)</div>"
...
"<button class='picker-btn' onclick='setDieFromPicker(1)'>1</button>"
"<button class='picker-btn' onclick='setDieFromPicker(2)'>2</button>"
"<button class='picker-btn' onclick='setDieFromPicker(3)'>3</button>"
"<button class='picker-btn' onclick='setDieFromPicker(4)'>4</button>"
"<button class='picker-btn' onclick='setDieFromPicker(5)'>5</button>"
"<button class='picker-btn' onclick='setDieFromPicker(6)'>6</button>"
"</div>"
...
"function setDieFromPicker(value){if(currentPickerIndex===null)return;gameState.dice[currentPickerIndex]=value;updateDiceVisual(currentPickerIndex);closePicker();checkAllDiceEntered();}"
...
"function clearDice(){gameState.dice=[0,0,0,0,0];gameState.held=[false,false,false,false,false];analyzeBtn.style.display='none';nextRollBtn.style.display='none';doneBtn.style.display='none';rollInfoEl.textContent='Tap a die to choose its value (1-6)';recommendationsContainer.innerHTML='<p style=\"text-align: center; color: #999; font-size: 0.95em;\">Enter your dice to see the optimal strategy!</p>';for(var i=0;i<5;i++){updateDiceVisual(i);}updateBonusTracker();updateTotalScore();}"
"function checkAllDiceEntered(){var allSet=gameState.dice.every(function(d){return d>0;});if(allSet){analyzeBtn.style.display='inline-block';rollInfoEl.textContent='All dice set. Tap \"Analyze Roll\" to see recommendations.';}else{analyzeBtn.style.display='none';rollInfoEl.textContent='Tap a die to choose its value (1-6)';}}"
...
"function nextRoll(){gameState.dice=[0,0,0,0,0];gameState.held=[false,false,false,false,false];analyzeBtn.style.display='none';nextRollBtn.style.display='none';doneBtn.style.display='none';for(var i=0;i<5;i++){updateDiceVisual(i);}rollInfoEl.textContent='Enter dice for roll '+(gameState.rollNumber+1)+'/3, then tap \"Analyze Roll\".';recommendationsContainer.innerHTML='<p style=\"text-align: center; color: #999; font-size: 0.95em;\">Enter your dice to see the optimal strategy!</p>';updateBonusTracker();updateTotalScore();}"
