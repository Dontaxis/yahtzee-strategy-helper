...
"<div class='roll-info' id='rollInfo'>Tap a die to cycle through values (1-6)</div>"
...
"var diceDivs=[],holdButtons=[],recommendationsContainer,rollInfoEl,analyzeBtn,nextRollBtn,doneBtn,bonusFill,bonusText,totalScoreEl;"
"function initUI(){"
...
"die.dataset.index=i;"
"die.textContent='?';"
"die.addEventListener('click',function(idx){return function(){cycleDie(idx);};}(i));"
...
"updateDisplay();}"
"function cycleDie(index){var currentValue=gameState.dice[index];gameState.dice[index]=(currentValue%6)+1;updateDiceVisual(index);checkAllDiceEntered();}"
...
"function clearDice(){gameState.dice=[0,0,0,0,0];gameState.held=[false,false,false,false,false];analyzeBtn.style.display='none';nextRollBtn.style.display='none';doneBtn.style.display='none';rollInfoEl.textContent='Tap a die to cycle through values (1-6)';recommendationsContainer.innerHTML='<p style=\"text-align: center; color: #999; font-size: 0.95em;\">Enter your dice to see the optimal strategy!</p>';for(var i=0;i<5;i++){updateDiceVisual(i);}updateBonusTracker();updateTotalScore();}"
"function checkAllDiceEntered(){var allSet=gameState.dice.every(function(d){return d>0;});if(allSet){analyzeBtn.style.display='inline-block';rollInfoEl.textContent='All dice set. Tap \"Analyze Roll\" to see recommendations.';}else{analyzeBtn.style.display='none';rollInfoEl.textContent='Tap a die to cycle through values (1-6)';}}"
