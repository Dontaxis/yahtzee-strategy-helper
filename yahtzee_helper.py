...
".picker-dialog { background: #ffffff; border-radius: 16px; padding: 24px; width: 280px; box-shadow: 0 10px 30px rgba(0,0,0,0.4); text-align: center; animation: popIn 0.25s ease-out; }"
"@keyframes popIn { from { opacity: 0; transform: scale(0.9); } to { opacity: 1; transform: scale(1); } }"
".picker-title { font-weight: bold; margin-bottom: 16px; color: #333; font-size: 1.3em; }"
".picker-input { width: 100%; padding: 16px; font-size: 2.5em; text-align: center; border: 3px solid #764ba2; border-radius: 12px; margin-bottom: 16px; font-weight: bold; color: #764ba2; background: #f7f5ff; }"
".picker-input:focus { outline: none; border-color: #667eea; background: #fff; }"
".picker-buttons { display: flex; gap: 10px; }"
".picker-ok { flex: 1; padding: 14px 20px; font-size: 1.1em; border-radius: 8px; border: none; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; cursor: pointer; font-weight: bold; }"
".picker-cancel { flex: 1; padding: 14px 20px; font-size: 1.1em; border-radius: 8px; border: 2px solid #bbb; background: #f5f5f5; cursor: pointer; font-weight: bold; }"
"</style>"
"</head>"
"<body>"
"<div class='container'>"
...
"<div class='picker-title'>Enter die value (1-6)</div>"
"<input type='tel' id='pickerInput' class='picker-input' inputmode='numeric' pattern='[1-6]' min='1' max='6' autocomplete='off' />"
"<div class='picker-buttons'>"
"<button class='picker-ok' onclick='confirmPickerValue()'>OK</button>"
...
"var diceDivs=[],holdButtons=[],recommendationsContainer,rollInfoEl,analyzeBtn,nextRollBtn,doneBtn,bonusFill,bonusText,totalScoreEl,pickerOverlay,pickerInput,currentPickerIndex=null;"
"function initUI(){"
...
"pickerInput.addEventListener('keypress',function(e){if(e.key==='Enter'){confirmPickerValue();}});"
"for(var i=0;i<5;i++){"
...
"updateDisplay();}"
"function openPicker(index){currentPickerIndex=index;pickerInput.value='';pickerOverlay.style.display='flex';pickerInput.focus();setTimeout(function(){pickerInput.focus();},10);setTimeout(function(){pickerInput.focus();},50);setTimeout(function(){pickerInput.focus();},100);setTimeout(function(){pickerInput.focus();},200);}"
"function closePicker(){pickerOverlay.style.display='none';currentPickerIndex=null;pickerInput.value='';}"
"function confirmPickerValue(){if(currentPickerIndex===null)return;var value=parseInt(pickerInput.value);if(value>=1&&value<=6){gameState.dice[currentPickerIndex]=value;updateDiceVisual(currentPickerIndex);closePicker();checkAllDiceEntered();autoAdvanceToNextDie(currentPickerIndex);}else{alert('Please enter a number between 1 and 6');}}"
"function autoAdvanceToNextDie(currentIndex){for(var i=currentIndex+1;i<5;i++){if(gameState.dice[i]===0){setTimeout(function(idx){return function(){openPicker(idx);};}(i),250);return;}}}"
